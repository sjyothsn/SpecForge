...
class SpecInsightViewProvider implements vscode.WebviewViewProvider {
  public static readonly viewType = "specinsight.panel";
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider(SpecInsightViewProvider.viewType, provider)
  );

  context.subscriptions.push(
    vscode.commands.registerCommand("specinsight.run", async () => {
      await runSpecInsight("examples", "outputs");
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand("specinsight.openLatestReport", async () => {
...
async function runSpecInsight(inputPathText: string, outputPathText: string): Promise<void> {
  ...
  const pythonPath = vscode.workspace.getConfiguration().get<string>("specinsight.pythonPath", "python");
  const mode = vscode.workspace.getConfiguration().get<string>("specinsight.pyPathMode", "env");
...
async function openLatestReport(outputPathText: string): Promise<void> {
  const workspace = vscode.workspace.workspaceFolders?.[0];
  if (!workspace) {
  ...
  const files = await vscode.workspace.findFiles(
    new vscode.RelativePattern(outputRoot, "**/report.html"),
    "**/node_modules/**",
    1
  );
